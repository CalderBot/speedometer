...
// Typical usage is that the gauge is a spedometer.
// Note: REQUIRES Raphael.js


// Gauge consructor.  divNameStr is the name of the div to contian the gauge.  units (e.g. "mph") is the name of the  tracked quantity.  w and h are the dimensions of the gauge. min/maxValue describe the range of values to display.  current/max/ave are the current/max/average of the value.  timeUnits is milliseconds per unit of time in measurement (e.g. 1000 if value is in meters per *second*)   
var Gauge = function(divNameStr,units,w,h,minValue,maxValue,current,max,ave,timeUnits){
    ...
    var TICKCOLOR = "magenta";     // Color of tickmarks
    ...
    var maxPath, avePath, currentPath; //  The dials which display current, max, ave
  ...
  //  var s = this.r.set();
    ...
    while (angle <= 360-ANGLE) {
        (function (t,ang,self) {
            ...
            // Every fifth tickmark is longer, and displays a clickable value:
                ...
                self.r.path("M"+cx+","+(cy+R-3*TICKLENGTH-TEXTHEIGHT)+"l0,"+(1.5*TICKLENGTH)).attr({ stroke: TICKCOLOR, "stroke-width":"2", transform: t})
    ...
    // Create a maxPath and currentPath:        
    maxPath = this.r.path("M"+cx+","+(cy+CR)+"l0,"+(R-CR-TICKLENGTH-1.3*TEXTHEIGHT)).attr({fill: "none", "stroke-width": 3, stroke:MAX_HAND_COLOR, transform: "r"+(ANGLE+radians*this.max)+","+cx+","+cy})
    avePath = this.r.path("M"+cx+","+(cy+CR)+"l0,"+(R-CR-TICKLENGTH-1.3*TEXTHEIGHT)).attr({fill: "none", "stroke-width": 3, stroke:AVE_HAND_COLOR, transform: "r"+(ANGLE+radians*this.ave)+","+cx+","+cy})
    currentPath = this.r.path("M"+cx+","+(cy+CR)+"l0,"+(R-CR-TICKLENGTH-1.3*TEXTHEIGHT)).attr({fill: "none", "stroke-width": 4, stroke:CURRENT_HAND_COLOR, transform: "r"+(ANGLE+radians*this.current)+","+cx+","+cy});
    ...
    this.update = function(cur){
        ...
        if(this.currentTime === this.startTime){ this.ave = this.current; }
        // Given average values a1, a2 over durations t1,t2, their combined average is (a1*t1+a2*t2)/(t1+t2)
        else{ this.ave = (this.ave*(this.currentTime-this.startTime)+this.current*this.timeUnits)/(now-this.startTime); }
